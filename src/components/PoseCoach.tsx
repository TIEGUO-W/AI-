...
export default function PoseCoach() {
  ...
  // ─── TTS 播放辅助：fetch+blob 绕过跨域 autoplay 限制 ───
  const playAudioUrl = useCallback(async (audioUrl: string) => {
    try {
      if (audioRef.current) {
        audioRef.current.pause();
        const old = audioRef.current as HTMLAudioElement & {_blobUrl?: string};
  ...
  const toggleVoiceMode = useCallback(() => {
    if (voiceEnabled) {
      ...
      if (SpeechRec) {
        ...
        rec.onerror = () => {
          if (voiceActiveRef.current) { setTimeout(() => { try { rec.start(); } catch {} }, 1000); }
        };
        ...
        const startRec = async () => {
          try {
            ...
            recorder.ondataavailable = (e) => {
              if (e.data.size > 0 && voiceActiveRef.current) {
  ...
  const handleWsMessage = useCallback((msg: WsMessage) => {
    ...
    if (msg.type === 'voice_recognized') {
      ...
      setVoiceMessages(prev => [...prev, { from: 'user' as const, text: payload.text }].slice(-10));
    }

    // 语音回复文字
    if (msg.type === 'voice_reply') {
      const payload = msg.payload as { text: string };
      setVoiceMessages(prev => [...prev, { from: 'coach' as const, text: payload.text }].slice(-10));
    }

    // 语音回复TTS
    if (msg.type === 'voice_reply_tts') {
      const payload = msg.payload as { audioUrl: string };
      if (payload.audioUrl) {
        playAudioUrl(payload.audioUrl);
      }
    }
      ...
      <div className="flex w-[380px] shrink-0 flex-col border-l border-[#1A1D27] bg-[#0F1117]">
        ...
        <div className="border-t border-[#1A1D27] px-5 py-3">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-[#8B8FA3]">语音控制</span>
            <button
              onClick={toggleVoiceMode}
              className={`text-xs px-3 py-1 rounded-full transition-all ${
                voiceEnabled
                  ? 'bg-[#FF6B35] text-white shadow-[0_0_12px_rgba(255,107,53,0.4)]'
                  : 'bg-[#1A1D27] text-[#8B8FA3] hover:text-[#E8E9ED]'
              }`}
            >
              {voiceEnabled ? '监听中' : '点击开启'}
            </button>
          </div>
          {voiceEnabled && (
            <div className="space-y-1.5 max-h-24 overflow-y-auto">
              {voiceMessages.length === 0 ? (
                ...
                voiceMessages.slice(-4).map((msg, i) => (
                  <div key={i} className={`text-[10px] ${msg.from === 'user' ? 'text-[#FF6B35]' : 'text-[#22D3A7]'}`}>
        ...
        {/* 架构说明 */}
        <div className="border-t border-[#1A1D27] px-5 py-2">
          <div className="text-[10px] text-[#8B8FA3]/50 leading-relaxed">
            {source === 'local'
              ? '浏览器 MediaPipe → 云端算法 + LLM + TTS'
              : 'RPi 摄像头 → 云端骨架检测 + LLM + TTS'
            }
          </div>
        </div>
      </div>
