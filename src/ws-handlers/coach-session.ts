...
const DEFAULT_CONFIG: SessionConfig = {
  llmTimeoutMs: 3500,
  speechCooldownMs: 5000,       // 说话冷却 5s（TTS需要时间播放，不能太密集）
  idleThresholdMs: 8000,        // 8秒不动就算空闲
  deepCoachIntervalMs: 45_000,
  maxHistoryLength: 20,
};
...
export class CoachSession {
  ...
  private startTimers(): void {
    ...
    this.idleTimer = setInterval(() => {
      ...
      if (idle < this.config.idleThresholdMs && timeSinceLastSpeech > 15000) {
  ...
  private shouldSpeak(result: AlgorithmResult): SpeechDecision {
    ...
    // 5. 完成一次 → 中等概率鼓励
    if (result.completedRep) {
      const speakChance = result.repCount <= 3 ? 0.6   // 前3次 60%
        : result.repCount <= 10 ? 0.35                   // 4-10次 35%
        : 0.2;                                           // 10次以上 20%
    ...
    // 6. 周期性鼓励 — 用户在做动作但教练长时间没说话（12秒+）
    if (this.recentQualityScores.length >= 2 && now - this.lastSpeechTime > 12000) {
      const q = result.quality.qualityScore >= 85 ? 'good' as const
        : result.quality.qualityScore >= 60 ? 'adjust' as const
        : 'warning' as const;
      const fb = generateQuickCoaching(result.exercise, result.stage, q, result.repCount);
